...
def plot_wing_3d(
    ...
    """
    3D lifting-surface visualization (LLT representation).
    Single ruled surface — no airfoil thickness, no duplicate geometry.
    Grid shape: (2, N_span).
    """
    b = span / 2.0
    ...
    # Chord: tapers symmetrically (abs is correct — both tips taper)
    chord = chord_root + (chord_tip - chord_root) * (np.abs(y) / b)

    # Sweep: signed y (no abs — left/right sweep correctly)
    x_le = y * np.tan(sweep_rad)

    # Trailing edge: derived from leading edge
    x_te = x_le + chord

    # Dihedral: both wings rise from center (abs is correct)
    z_base = np.tan(dihedral_rad) * np.abs(y)

    # --- Build ruled-surface grid: shape (2, N_span) ---
    X = np.vstack([x_le, x_te])      # (2, N)
    Y = np.vstack([y, y])            # (2, N)
    Z = np.vstack([z_base, z_base])  # (2, N)

    # --- Twist: rotation about quarter-chord ---
    for i in range(N_span):
        twist_local = twist_deg * (y[i] / b)  # signed y — no abs
    ...
    # --- Cl color mapping: shape (2, N_span) ---
    surf_color = None
    show_colorbar = False
    ...
    # --- Debug validation ---
    print(f"[Wing3D] X={X.shape} Y={Y.shape} Z={Z.shape}")

    # --- Build figure: ONE surface ---
    fig = go.Figure()
    ...
    # Validate: must be exactly 1 surface
